...
import {
  ArrowLeft, CheckCircle, AlertTriangle, XCircle,
  Hash, Shield, Download, FileText // Added FileText for PDF icon
} from 'lucide-react'
import CircularScore    from '../components/CircularScore.jsx'
import BlockchainBadge  from '../components/BlockchainBadge.jsx'
import SourceList        from '../components/SourceList.jsx'
...
export default function ReportDetails() {
  const navigate = useNavigate()
  const { reportId: urlReportId } = useParams() // Get ID from URL if available
  const [result, setResult] = useState(null)
  const [tab,    setTab]    = useState('overview')
  ...
  const handleDownloadPDF = () => {
    const id = result?.report_id || result?.id || urlReportId;
    if (!id) return alert("Report ID not found.");
    
    // Redirect to the Flask backend endpoint we created
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="card">
            ...
            {[
              ...
              <div key={label} className="mb-4">
                <div className="font-jetbrains text-app3 uppercase mb-0.5" style={{ fontSize: 10 }}>
                  {label}
                </div>
                <div className="flex items-center font-jetbrains text-app" style={{ fontSize: 12 }}>
